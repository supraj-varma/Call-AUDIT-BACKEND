...
def get_jwt_secret() -> str:
    ...
    if not secret:
        # Fallback for local dev if needed, but raise in prod-like checks if required
        return "super-secret-jwt-token-key-change-this"
    return secret
...
async def list_users(admin: dict = Depends(require_admin)):
    try:
        ...
        supabase = get_supabase()
        admins = supabase.table("admin_users").select("id, email, name, created_at").execute().data
        members = supabase.table("members").select("id, email, name, created_at").execute().data
        return {
            "admins": admins,
            "members": members
        }
...
async def delete_member(member_id: str, admin: dict = Depends(require_admin)):
    """Admin-only: permanently removes a member account."""
    try:
        supabase = get_supabase()
